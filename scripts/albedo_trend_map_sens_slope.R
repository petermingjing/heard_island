...
# Pixel-level Sen's slope trend analysis for multiple sensors
...
# Function to calculate Sen's slope for a pixel time series
calculate_sens_slope_pixel <- function(values, years) {
  # Remove NA values
  ...
  # Calculate Sen's slope using zyp package
  tryCatch({
    sens_result <- zyp.sen(valid_values ~ valid_years)
    slope <- sens_result$coeff[2]  # Slope per year
    
    # Calculate Mann-Kendall test for significance
...
create_trend_map <- function(trend_results, sensor_name) {
  ...
  p <- ggplot(plot_df, aes(x = x, y = y)) +
    ...
    labs(
      ...
      subtitle = paste("Sen's slope per year (", min(trend_results$years), "-", max(trend_results$years), ")"),
      x = "Longitude",
      y = "Latitude"
    ) +
    theme_times() +
...
cat("Pixel-level Sen's slope trend analysis for multiple sensors\n")
cat(paste(rep("=", 80), collapse=""), "\n")
  ...
  if (nrow(summary_data) > 1) {
    p_combined <- ggplot(summary_data, aes(x = sensor, y = mean_slope)) +
      ...
      labs(
        ...
        subtitle = "Sen's slope per year with standard deviation",
        x = "Sensor",
...
cat("  • Sen's slope estimator (robust trend detection)\n")
