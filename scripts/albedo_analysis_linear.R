...
calculate_robust_trends <- function(data, sensor_name) {
  ...
  if (nrow(annual_means) >= 4) {
    # 1. Ordinary least squares (primary method)
    ...
    bootstrap_ci <- function(data, n_bootstrap = 1000) {
      ...
      for (i in 1:n_bootstrap) {
        ...
        if (nrow(boot_sample) >= 4) {
          boot_model <- lm(mean_albedo ~ year, data = boot_sample)
          slopes[i] <- coef(boot_model)[2] * 10
        }
      }
    ...
    return(list(
      ...
      bootstrap_ci_lower = ci_95[1],
      bootstrap_ci_upper = ci_95[2],
...
cat("\nAnalyzing temporal trends using linear regression...\n")
trends <- list()
...
for (sensor in unique(all_albedo_data$sensor)) {
  ...
  if (!is.null(trend_result)) {
    ...
    cat(sprintf("  %s:\n", sensor))
    cat(sprintf("    OLS slope: %.4f per decade [%.4f, %.4f] (95%% CI)\n",
                trend_result$ols_slope, 
                trend_result$ols_ci_lower, 
                trend_result$ols_ci_upper))
    cat(sprintf("    OLS: R²=%.3f, p=%.3f%s\n",
                trend_result$ols_r_squared, trend_result$ols_p_value, ols_significance))
...
if (length(trends) > 0) {
  trend_summary <- data.frame(
    sensor = names(trends),
    ols_slope_per_decade = sapply(trends, function(x) x$ols_slope),
    ols_ci_lower = sapply(trends, function(x) x$ols_ci_lower),
    ols_ci_upper = sapply(trends, function(x) x$ols_ci_upper),
    ols_r_squared = sapply(trends, function(x) x$ols_r_squared),
    ols_p_value = sapply(trends, function(x) x$ols_p_value),
    mk_tau = sapply(trends, function(x) x$mk_tau),
    mk_p_value = sapply(trends, function(x) x$mk_p_value),
    n_years = sapply(trends, function(x) x$n_years),
    max_autocorrelation = sapply(trends, function(x) x$max_autocorrelation),
    stringsAsFactors = FALSE
  )
  write.csv(trend_summary, 
            "output/trend_analysis_results.csv", 
            row.names = FALSE)
  cat("✓ Exported trend analysis results: output/trend_analysis_results.csv\n")
}
...
# Export climate correlation results
if (length(climate_correlations) > 0) {
  climate_corr_summary <- data.frame()
  for (sensor_name in names(climate_correlations)) {
    corr_data <- climate_correlations[[sensor_name]]
    sensor_corr <- data.frame(
      sensor = sensor_name,
      variable = c("temperature", "precipitation", "snowfall", "radiation",
                   "partial_temperature", "partial_precipitation", "partial_snowfall", "partial_radiation"),
      correlation = c(corr_data$temperature$cor, corr_data$precipitation$cor, 
                     corr_data$snowfall$cor, corr_data$radiation$cor,
                     corr_data$partial_temperature$cor, corr_data$partial_precipitation$cor,
                     corr_data$partial_snowfall$cor, corr_data$partial_radiation$cor),
      p_value = c(corr_data$temperature$p_value, corr_data$precipitation$p_value,
                 corr_data$snowfall$p_value, corr_data$radiation$p_value,
                 corr_data$partial_temperature$p_value, corr_data$partial_precipitation$p_value,
                 corr_data$partial_snowfall$p_value, corr_data$partial_radiation$p_value),
      fdr_p_value = c(corr_data$temperature$fdr_p_value, corr_data$precipitation$fdr_p_value,
                     corr_data$snowfall$fdr_p_value, corr_data$radiation$fdr_p_value,
                     corr_data$partial_temperature$fdr_p_value, corr_data$partial_precipitation$fdr_p_value,
                     corr_data$partial_snowfall$fdr_p_value, corr_data$partial_radiation$fdr_p_value),
      stringsAsFactors = FALSE
    )
    climate_corr_summary <- rbind(climate_corr_summary, sensor_corr)
  }
  write.csv(climate_corr_summary, 
            "output/climate_correlation_results.csv", 
            row.names = FALSE)
  cat("✓ Exported climate correlation results: output/climate_correlation_results.csv\n")
}
...
write.csv(all_albedo_data[, c("date", "albedo", "sensor", "year", "month", "season")], 
          "output/time_series_data.csv", 
          row.names = FALSE)
cat("✓ Exported time series data: output/time_series_data.csv\n")
...
p1 <- ggplot(all_albedo_data, aes(x = date, y = albedo, color = sensor)) +
  ...
  theme(legend.position = "bottom",
        ...
        plot.margin = ggplot2::margin(b = 40, unit = "pt"))
...
# Export annual trends data
write.csv(annual_data, 
          "output/annual_trends_data.csv", 
          row.names = FALSE)
cat("✓ Exported annual trends data: output/annual_trends_data.csv\n")
...
trend_annotations <- data.frame(
  ...
  label = sapply(names(trends), function(s) {
    if (s %in% names(trends)) {
      ...
      sprintf("%s: %.3f/dec%s\nR²=%.3f, p=%.3f", 
              s, trend_data$ols_slope, mk_sig, 
              trend_data$ols_r_squared, trend_data$ols_p_value)
    } else {
      sprintf("%s: No data", s)
    }
  })
...
p2 <- ggplot(annual_data, aes(x = year, y = mean_albedo, color = sensor)) +
  ...
  theme_times() +
  theme(legend.position = "bottom",
        legend.margin = ggplot2::margin(t = 20, unit = "pt"),
        plot.margin = ggplot2::margin(b = 60, unit = "pt"))
...
# Export seasonal patterns data
write.csv(seasonal_results$monthly, 
          "output/seasonal_patterns_monthly_data.csv", 
          row.names = FALSE)
write.csv(seasonal_results$seasonal, 
          "output/seasonal_patterns_seasonal_data.csv", 
          row.names = FALSE)
cat("✓ Exported seasonal patterns data\n")
...
if ("VIIRS" %in% names(climate_correlations)) {
  ...
  # Export climate correlation data
  write.csv(viirs_climate, 
            "output/climate_correlation_data.csv", 
            row.names = FALSE)
  cat("✓ Exported climate correlation data: output/climate_correlation_data.csv\n")
...
# Export data availability (counts by date)
data_availability <- all_albedo_data %>%
  group_by(date) %>%
  summarise(n_observations = n(), .groups = "drop")
write.csv(data_availability, 
          "output/data_availability_data.csv", 
          row.names = FALSE)
cat("✓ Exported data availability data: output/data_availability_data.csv\n")
...
# Export climate time series data
write.csv(climate_annual, 
          "output/climate_time_series_data.csv", 
          row.names = FALSE)
cat("✓ Exported climate time series data: output/climate_time_series_data.csv\n")
...
# Export albedo distribution data (already in all_albedo_data, but create histogram summary)
albedo_dist_summary <- all_albedo_data %>%
  group_by(sensor) %>%
  summarise(
    albedo_value = albedo,
    .groups = "drop"
  )
write.csv(albedo_dist_summary, 
          "output/albedo_distribution_data.csv", 
          row.names = FALSE)
cat("✓ Exported albedo distribution data: output/albedo_distribution_data.csv\n")
...
write.csv(summary_stats, 
          "output/summary_statistics_data.csv", 
          row.names = FALSE)
cat("✓ Exported summary statistics data: output/summary_statistics_data.csv\n")
...
for (sensor in names(trends)) {
  ...
  cat(sprintf("  %s:\n", sensor))
  cat(sprintf("    OLS slope: %.4f [%.4f, %.4f] per decade (95%% CI)\n",
              trend_data$ols_slope, trend_data$ols_ci_lower, trend_data$ols_ci_upper))
  cat(sprintf("    OLS: R²=%.3f, p=%.3f%s\n",
              trend_data$ols_r_squared, trend_data$ols_p_value, ols_significance))
  cat(sprintf("    Mann-Kendall: τ=%.3f, p=%.3f%s\n",
...
cat("\nData exports saved:\n")
cat("  • Time series data: output/time_series_data.csv\n")
cat("  • Annual trends data: output/annual_trends_data.csv\n")
cat("  • Seasonal patterns: output/seasonal_patterns_monthly_data.csv, output/seasonal_patterns_seasonal_data.csv\n")
cat("  • Climate correlation data: output/climate_correlation_data.csv\n")
cat("  • Climate correlation results: output/climate_correlation_results.csv\n")
cat("  • Data availability: output/data_availability_data.csv\n")
cat("  • Climate time series: output/climate_time_series_data.csv\n")
cat("  • Albedo distribution: output/albedo_distribution_data.csv\n")
cat("  • Summary statistics: output/summary_statistics_data.csv\n")
cat("  • Trend analysis results: output/trend_analysis_results.csv\n")
